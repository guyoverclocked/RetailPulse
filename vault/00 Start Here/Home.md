...
```mermaid
flowchart LR
    A[Project definition] --> B[Data foundation]
    B --> C[Honest evaluation]
    C --> D[Global probabilistic model]
    D --> E[Challenger selection]
    E --> F[Staffing decision]
    F --> G[API and dashboard]
    G --> H[Automation and monitoring]
    H --> I[Portfolio release]
```
